#!/bin/bash
#SBATCH --job-name=rna_protein_transformer_evaluate
#SBATCH --output=rna_protein_transformer_evaluate_%j.out
#SBATCH --error=rna_protein_transformer_evaluate_%j.err
...
if [ ! -f "outputs_RPI1807_transformer/checkpoints/best_model.pth" ]; then
    ...
    echo "Please run training first: sbatch job/Train/slurm_rna_protein_transformer.sh"
    exit 1
fi
...
echo "Transformer evaluation completed at: $(date)"
